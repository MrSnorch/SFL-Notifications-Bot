...
def process_ready_alerts(chat_id: int, events: list[Event],
    ...
    for e in events:
        ...
        if stored is None:
            ...
            if old_rc == 0 and e.ready_count > 0:
                # Настоящая новая волна: было 0 готовых, теперь появились — удаляем старое, шлём новое (с пингом)
            ...
            elif e.ready_count != old_rc or e.count > old_c:
                # Изменилось количество готового (выросло в рамках текущей волны либо игрок собрал часть) — редактируем без пинга
                tg_edit(TG_TOKEN, chat_id, mid, text,
                        reply_markup=_dismiss_keyboard(key))
                alerts_state[key] = {**stored, "ready_count": e.ready_count, "count": e.count}
                log.info(f"[{chat_id}] Алерт «{key}»: {old_rc}→{e.ready_count}/{e.count} — сообщение отредактировано")
...
def scan_user(user: dict) -> "int | None":
    ...
    # ── Daily Rewards ─────────────────────────────────────────────────────────
    if tracking.get("daily_reward", True):
